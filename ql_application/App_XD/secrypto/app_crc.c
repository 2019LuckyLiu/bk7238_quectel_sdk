...
*	data: source data
*   len: source data len
*   flag: 0:check uart data fram is valide, 1-calc crc value
*/
...
{	
	...
	{
		crcCal ^= data[n];
		for(i=0; i<8; i++)//8 means type char
		{
			if(crcCal&0x01)
			{
				crcCal >>= 1;
	...
	if(flag==0) //check crc right or not
	{
		crcData =(data[len-2]<<8) | data[len-1];
		if(crcCal!= crcData)
		{
			//printf("[slave uart] crcError recv_Cal = %04x_%04x\r\n", crcData, crcCal);
			crcCal = 0x00;
		}
		else if((crcCal == crcData)&&(crcCal==0))
		{
			crcCal = 0x01;
		}
	}
