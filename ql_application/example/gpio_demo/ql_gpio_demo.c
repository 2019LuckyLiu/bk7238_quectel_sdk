...
void ql_gpio_demo_thread(void *param)
{
	...
	ql_gpio_log("[mcu led]:gpio  test start \r\n");
	ret = ql_gpio_init(QL_GPIO23,QL_GMODE_OUTPUT,QL_GPIO_LEVEL_LOW);
    if(ret == QL_GPIO_SUCCESS)
	  ql_gpio_log("[mcu led]:gpio23 gpio init success\r\n");
	else
	  ql_gpio_log("[mcu led]:gpio init fail\r\n");

	while(1)
	{
	  ql_rtos_task_sleep_ms(500);
	  ql_gpio_set_level(QL_GPIO23,QL_GPIO_LEVEL_HIGH);
	  ql_rtos_task_sleep_ms(500);
	  ql_gpio_set_level(QL_GPIO23,QL_GPIO_LEVEL_LOW);
	}
}
